...
async function getSingleIP(hostname: string): Promise<string> {
    ...
    return new Promise((resolve, reject) => {
        originalResolve4(hostname, (err, addresses) => {
...
// 1. Patch dns.lookup (callback style)
...
// 2. Patch dns.resolve4 (direct resolution)
...
// 3. Patch dns.promises.resolve4 if it exists
if (dns.promises && dns.promises.resolve4) {
    const originalPromisesResolve4 = dns.promises.resolve4;
    (dns.promises as any).resolve4 = async function(hostname: string, options?: any) {
        try {
...
// 4. Patch socket connections at the lowest level
...
console.log('[DNS-FIX] ✅ Patched all DNS resolution paths - Single IP mode enabled');
// ===== END DNS FIX =====
