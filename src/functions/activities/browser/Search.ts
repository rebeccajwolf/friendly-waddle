...
export class Search extends Workers {
    ...
    private async clickRandomResult(page: Page): Promise<void> {
        ...
        // TypeScript needs explicit check – selectedLink cannot be undefined here because we checked length > 0
        if (!selectedLink) {
            this.bot.logger.warn(this.bot.isMobile, 'SEARCH-RANDOM-CLICK', 'Selected link is unexpectedly null');
            return;
        }
